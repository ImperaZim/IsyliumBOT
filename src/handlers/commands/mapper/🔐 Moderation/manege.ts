...
import { ApplicationCommandType, ActionRowBuilder, StringSelectMenuBuilder } from "discord.js";
...
export default new ExtendedCommand({
  ...
  async run({ interaction }: CommandProps) {
    ...
    if (data) {
      ...
      const controller = new SettingsController(user, guild);
      await controller.startSettingsInteraction(response);
    }
  }
});
